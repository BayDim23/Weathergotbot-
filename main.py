...
from aiogram.types import Message  # Используйте этот импорт для Message
...
async def check_subscription(message: Message):  # Исправленный импорт
    CHANNEL_ID = None
    # Получаем статус пользователя в группе/канале
    ...
    if chat_member.status in [ChatMemberStatus.MEMBER, ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
        await message.answer("Спасибо, что присоединились к нашему каналу! Теперь вы можете продолжить.")
        # Здесь можно позволить пользователю продолжить выполнение действия или перейти на следующую страницу
    else:
        await message.answer("Чтобы продолжить, пожалуйста, подпишитесь на наш канал: @your_channel_or_group_link")
        # Здесь можно отправить сообщение с просьбой подписаться и заблокировать дальнейшие действия
